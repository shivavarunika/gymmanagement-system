...
const AdminPackageManager = () => {
  ...
  const handleAddPackage = (e) => {
    ...
    if (!packageName || !packagePrice || !packageDescription || !packageImage) return;
    ...
    <div className="min-h-screen p-6 bg-gray-100">
      ...
      <div className="mt-8 max-w-4xl mx-auto bg-white p-6 rounded shadow">
        ...
        <form onSubmit={handleAddPackage} className="space-y-4">
          ...
          <div>
            <label className="block text-lg font-semibold mb-1">Package Image URL</label>
            <input
              type="text"
              className="w-full p-2 border rounded"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {packages.map((pkg, index) => (
                ...
                >
                  ...
                  />
                  <h3 className="font-bold text-lg">{pkg.name} - ${pkg.price}</h3>
                  <p className="text-gray-700 mt-1 text-sm">{pkg.description}</p>
                </div>
              ))}
            </div>
          )}
